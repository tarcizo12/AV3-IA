...
def genetic_algorithm(board_size, population_size, num_generations, mutation_rate, optimal_fitness=None):
    ...
    for generation in range(num_generations):
        ...
        # Na etapa de recombinação, escolhe-se entre recombinação de um ponto ou dois pontos
        recombination_prob = random.uniform(0.85, 0.95)  # Probabilidade entre 85% e 95%
        if random.uniform(0, 1) < recombination_prob:
            child1, child2 = crossover_one_point(parents)
        else:
            child1, child2 = crossover_two_points(parents)
...
while solutions_found < 92:
    ...
    # Verifique se a solução é única
    if tuple(best_solution) not in found_solutions:
        found_solutions.add(tuple(best_solution))
        solutions_found += 1
        with open("solutions.txt", "a") as f:
            f.write(str(best_solution) + "\n")

print(f"Encontradas {solutions_found} soluções diferentes.")
